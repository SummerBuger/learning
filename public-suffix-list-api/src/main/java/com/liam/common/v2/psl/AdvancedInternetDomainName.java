 ...
 *
 *      在 {@link com.google.common.net.InternetDomainName} 的基础上，定时从<br />
 *      https://publicsuffix.org/list/public_suffix_list.dat <br />
 *      拉取最新的 <span color='red'>Public Suffix List</span> <br />
 *
 */
public final class AdvancedInternetDomainName {

  private static final Logger LOGGER = LoggerFactory.getLogger(AdvancedInternetDomainName.class);
   ...
   */
  AdvancedInternetDomainName(String name) {
    // Normalize:
    ...
    LOGGER.debug("to lower case used:{}", System.currentTimeMillis() - begin);
    begin = System.currentTimeMillis();
    if (name.endsWith(".")) {
      name = name.substring(0, name.length() - 1);
    }
    LOGGER.debug("rm latest point used:{}", System.currentTimeMillis() - begin);
    begin = System.currentTimeMillis();

    checkArgument(name.length() <= MAX_LENGTH, "Domain name too long: '%s':", name);
    this.name = name;
    LOGGER.debug("check and set name used:{}", System.currentTimeMillis() - begin);
    begin = System.currentTimeMillis();

    this.parts = ImmutableList.copyOf(DOT_SPLITTER.split(name));
    checkArgument(parts.size() <= MAX_PARTS, "Domain has too many parts: '%s'", name);
    checkArgument(validateSyntax(parts), "Not a valid domain name: '%s'", name);
    LOGGER.debug("check and set parts used:{}", System.currentTimeMillis() - begin);
    begin = System.currentTimeMillis();

    this.publicSuffixIndex = findPublicSuffix();
    LOGGER.debug("findPublicSuffix  used:{}", System.currentTimeMillis() - begin);
  }
  ...
  private int findPublicSuffix() {
    ...
    PublicSuffixSetCache publicSuffixListCache = PublicSuffixSetCache.newInstance();
    for (int i = 0; i < partsSize; i++) {
      String ancestorName = DOT_JOINER.join(parts.subList(i, partsSize));

      if (publicSuffixListCache.getExactPublicSuffixList().contains(ancestorName)) {
        return i;
      }
      ...
      if (publicSuffixListCache.getExcludePublicSuffixList().contains(ancestorName)) {
        return i + 1;
      }

      if (matchesWildcardPublicSuffix(ancestorName, publicSuffixListCache)) {
        return i;
      }
    }
  ...
  /**
   * Returns an newInstance of {@link AdvancedInternetDomainName} after lenient validation.
   ...
   */
  public static AdvancedInternetDomainName from(String domain) {
    return new AdvancedInternetDomainName(checkNotNull(domain));
  }
   ...
   * @since 6.0
   */
  public AdvancedInternetDomainName publicSuffix() {
   ...
   *
   * <p>
   * <b>Warning:</b> a {@code false} result from this method does not imply that the domain does not
   * represent an addressable host, as many public suffixes are also addressable hosts. Use
   ...
   *
   * <p>
   * <b>Warning:</b> A {@code true} result from this method does not imply that the domain is at the
   * highest level which is addressable as a host, as many public suffixes are also addressable
   ...
   *
   * <p>
   * If {@link #isTopPrivateDomain()} is true, the current domain name newInstance is returned.
   *
   * <p>
   * This method should not be used to determine the topmost parent domain which is addressable as a
   * host, as many public suffixes are also addressable hosts. For example, the domain
   ...
   * @since 6.0
   */
  public AdvancedInternetDomainName topPrivateDomain() {
  ...
  /**
   * Returns an {@code AdvancedInternetDomainName} that is the immediate ancestor of this one; that
   * is, the current domain with the leftmost part removed. For example, the parent of
   ...
   */
  public AdvancedInternetDomainName parent() {
   ...
   */
  private AdvancedInternetDomainName ancestor(int levels) {
    return from(DOT_JOINER.join(parts.subList(levels, parts.size())));
  }

  /**
   * Creates and returns a new {@code AdvancedInternetDomainName} by prepending the argument and a
   * dot to the current name. For example, {@code
   * AdvancedInternetDomainName.from("foo.com").child("www.bar")} returns a new
   * {@code AdvancedInternetDomainName} with the value {@code www.bar.foo.com}. Only lenient
   ...
   */
  public AdvancedInternetDomainName child(String leftParts) {
   ...
   * <pre>
   *    {@code
   *   domainName = AdvancedInternetDomainName.isValid(name)
   *       ? AdvancedInternetDomainName.from(name)
   *       : DEFAULT_DOMAIN;}
   * </pre>
   *
   * <pre>
   *    {@code
   *   try {
   *     domainName = AdvancedInternetDomainName.from(name);
  ...
  private static boolean matchesWildcardPublicSuffix(String domain,
      PublicSuffixSetCache publicSuffixListCache) {
    final String[] pieces = domain.split(DOT_REGEX, 2);
    return pieces.length == 2
        && publicSuffixListCache.getUnderPublicSuffixList().contains(pieces[1]);
  }
  ...
  public boolean equals(@Nullable Object object) {
    ...
    if (object instanceof AdvancedInternetDomainName) {
      AdvancedInternetDomainName that = (AdvancedInternetDomainName) object;
